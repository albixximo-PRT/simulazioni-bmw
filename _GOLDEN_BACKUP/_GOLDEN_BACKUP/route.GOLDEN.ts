...
function parseGaraFromColumnText(rawText: string): RaceRow[] {
  ...
  // esclude righe che sembrano chiaramente auto complete
...
function toExtractorCsv(rows: ExtractRow[]) {
  const header =
    "posGara,pilota,auto,tempoTotaleGara,distaccoDalPrimo,migliorGiroGara,tempoQualifica,pole"
  const body = rows
    .map((r) => [
      r.posGara,
      r.pilota,
...
function synthesizeRaceTimesForCsv(rows: ExtractRow[]): ExtractRow[] {
  ...
  const leader = out.find((r) => r.posGara === 1)
  const leaderMs = leader ? parseRaceTotalToMs(leader.tempoTotaleGara) : null
  if (leaderMs == null) return out

  let lastAbsMs = leaderMs
  let dnfCount = 0

  for (const r of out) {
    if (r.posGara === 1) {
      r.tempoTotaleGara = formatMsToRaceTime(leaderMs)
      continue
    }
    ...
    if (d.startsWith("+")) {
      ...
      if (gapMs != null) {
        const abs = leaderMs + gapMs
        r.tempoTotaleGara = formatMsToRaceTime(abs)
        lastAbsMs = abs
        continue
      }
    }
    ...
    if (d.toUpperCase() === "BOX") {
      r.tempoTotaleGara = formatMsToRaceTime(2 * 60 * 60 * 1000)
      continue
    }

    const fallback = lastAbsMs + 10000
    r.tempoTotaleGara = formatMsToRaceTime(fallback)
    lastAbsMs = fallback
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const csv = toExtractorCsv(outCsvRows)
    ...
    return Response.json({
      tool: "Albixximo Race Tools — Race CSV Extractor",
